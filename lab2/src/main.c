...
void task2LED() {
    ...
    // EACH CASE HAS A SIMILAR STRUCTURE
		if (delayCount >= 0) delayCount -- ; // decrement the counter
    switch (direction) {
        ...
        case ANTICLOCKWISE: /*MOVES THE CURRENT LIT LED BACKWARD */
            ...
            break ;
				case CLOCKWISE_INVERSE: /*INVEST OF CLOCKWISE ALL LEDS EXCEPT CURRENT ARE TURNED ON*/
						if (delayCount ==0){
							ledOnOff(leds[current],LED_ON);
							current=(current+1)%5;
							ledOnOff(leds[current],LED_OFF);
...
//    switch (stateLED5) {
//        case LED_OFF:  // the state names are defined in the gpio.h file
//            if (pressedB5_ev) {            // B5 has been pressed
//                pressedB5_ev = false ;     // acknowledge the event
//                ledOnOff (LED5, LED_ON) ;  // turn LED on
//                stateLED5 = LED_ON ;       // ... the new state
//            }
//            break ;
...
//            if (pressedB5_ev) {            // B5 has been pressed
//                pressedB5_ev = false ;     // acknowledge the event
//                stateLED5 = LED_OFF ;      // turn LED off
//                ledOnOff (LED5, LED_OFF) ; // ... the new state
//            }
//            break ;
//    }
//}
